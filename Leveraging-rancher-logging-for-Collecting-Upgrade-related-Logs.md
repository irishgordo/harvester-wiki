You might want to collect the logs from some upgrade-related key pods during the Harvester upgrade for whatever reason. A plain `kubectl logs` is not enough due to the unstableness of the cluster at the time. For example, a `kube-apiserver` restart will break the `kubectl logs`. Even worse, I occasionally see that the pod monitoring got torn down and spawn on a different node, the kubectl logs won't follow the newly spawned pod. But the worst part is, kubelet will only keep 1 recently generated log file. The pre-previous logs are purged, i.e you cannot access any logs older than a previous instantiation. It's very common that a pod is in a CrashLoppBackOff state during an upgrade, the intermediate logs are thus lost. You might ask why don't use the support bundle mechanism, that's because it only provides us the snapshot of logs of that specific moment. The problem is the same, we want the full picture of an upgrade.

The solution for this is to export the logs generated by the pods to the outside world through a logging infrastructure. Let's leverage the logging feature introduced in v1.1.0 and bring it to v1.0.3 for archiving all the upgrade-related logs during a Harvester upgrade. This will help collect all the logs across container restarts and pod evicts, even node reboots.
