...
* Bug (`area/bug`) issues are eligible for backporting. Issues with high severity (`severity/1`, `severity/2`) or `regression` must be backported unless they cause breaking changes or compatibility concerns. 
* For UI bug issues, `severity/3` issues are also considered for backporting, as the UI is the primary user interface.